// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using WebPetShop.Data;

#nullable disable

namespace WebPetShop.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20251126110551_Add_YeuCauNhanNuoi")]
    partial class Add_YeuCauNhanNuoi
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("WebPetShop.Models.BaiDangNhanNuoi", b =>
                {
                    b.Property<int>("MaBaiDang")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaBaiDang"));

                    b.Property<string>("HinhAnh")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("MaNguoiTao")
                        .HasColumnType("int");

                    b.Property<int>("MaThuCungNhanNuoi")
                        .HasColumnType("int");

                    b.Property<string>("MoTa")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<DateOnly?>("NgayDang")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("date")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("TieuDe")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("TrangThai")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasDefaultValue("Chờ duyệt");

                    b.HasKey("MaBaiDang")
                        .HasName("PK__BaiDangN__BF5D50C5D2EBC439");

                    b.HasIndex("MaNguoiTao");

                    b.HasIndex("MaThuCungNhanNuoi");

                    b.ToTable("BaiDangNhanNuoi", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.BaoCaoDoanhThu", b =>
                {
                    b.Property<int>("MaBaoCao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaBaoCao"));

                    b.Property<decimal?>("LoiNhuan")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<int?>("Nam")
                        .HasColumnType("int");

                    b.Property<DateTime?>("NgayTao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<int?>("NguoiLap")
                        .HasColumnType("int");

                    b.Property<int?>("Thang")
                        .HasColumnType("int");

                    b.Property<decimal?>("TongChiPhi")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<decimal?>("TongDoanhThu")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<int?>("TongDonHang")
                        .HasColumnType("int");

                    b.HasKey("MaBaoCao")
                        .HasName("PK__BaoCaoDo__25A9188C59576223");

                    b.HasIndex("NguoiLap");

                    b.ToTable("BaoCaoDoanhThu", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.ChiTietChamSoc", b =>
                {
                    b.Property<int>("MaCtcs")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTCS");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCtcs"));

                    b.Property<string>("GhiChu")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("HinhAnh")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("MaKyGui")
                        .HasColumnType("int");

                    b.Property<DateTime?>("NgayCapNhat")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("TinhTrang")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("MaCtcs")
                        .HasName("PK__ChiTietC__1E4E48C420DC318F");

                    b.HasIndex("MaKyGui");

                    b.ToTable("ChiTietChamSoc", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.ChiTietDonHang", b =>
                {
                    b.Property<int>("MaCtdh")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTDH");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCtdh"));

                    b.Property<decimal>("DonGia")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<int>("MaDh")
                        .HasColumnType("int")
                        .HasColumnName("MaDH");

                    b.Property<int>("MaSp")
                        .HasColumnType("int")
                        .HasColumnName("MaSP");

                    b.Property<int>("SoLuong")
                        .HasColumnType("int");

                    b.HasKey("MaCtdh")
                        .HasName("PK__ChiTietD__1E4E40F05C8E5744");

                    b.HasIndex("MaDh");

                    b.HasIndex("MaSp");

                    b.ToTable("ChiTietDonHang", null, t =>
                        {
                            t.HasTrigger("trg_CapNhatTonKho_AfterDonHang");
                        });

                    b.HasAnnotation("SqlServer:UseSqlOutputClause", false);
                });

            modelBuilder.Entity("WebPetShop.Models.ChiTietGioHang", b =>
                {
                    b.Property<int>("MaCtgh")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTGH");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCtgh"));

                    b.Property<int>("MaGh")
                        .HasColumnType("int")
                        .HasColumnName("MaGH");

                    b.Property<int>("MaSp")
                        .HasColumnType("int")
                        .HasColumnName("MaSP");

                    b.Property<int?>("SoLuong")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(1);

                    b.HasKey("MaCtgh")
                        .HasName("PK__ChiTietG__1E4FAF54BA8ECC03");

                    b.HasIndex("MaGh");

                    b.HasIndex("MaSp");

                    b.ToTable("ChiTietGioHang", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.ChiTietPhieuNhap", b =>
                {
                    b.Property<int>("MaCtpn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTPN");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCtpn"));

                    b.Property<decimal?>("DonGia")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<int?>("MaPn")
                        .HasColumnType("int")
                        .HasColumnName("MaPN");

                    b.Property<int?>("MaSp")
                        .HasColumnType("int")
                        .HasColumnName("MaSP");

                    b.Property<int?>("SoLuong")
                        .HasColumnType("int");

                    b.HasKey("MaCtpn")
                        .HasName("PK__ChiTietP__1E4E607516DCFB70");

                    b.HasIndex("MaPn");

                    b.HasIndex("MaSp");

                    b.ToTable("ChiTietPhieuNhap", null, t =>
                        {
                            t.HasTrigger("trg_CapNhatTonKhoSauNhapHang");
                        });

                    b.HasAnnotation("SqlServer:UseSqlOutputClause", false);
                });

            modelBuilder.Entity("WebPetShop.Models.ChiTietPhieuXuat", b =>
                {
                    b.Property<int>("MaCtpx")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaCTPX");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaCtpx"));

                    b.Property<decimal>("DonGia")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<int>("MaPx")
                        .HasColumnType("int")
                        .HasColumnName("MaPX");

                    b.Property<int>("MaSp")
                        .HasColumnType("int")
                        .HasColumnName("MaSP");

                    b.Property<int>("SoLuong")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(1);

                    b.HasKey("MaCtpx")
                        .HasName("PK__ChiTietPhieuXuat");

                    b.HasIndex("MaPx");

                    b.HasIndex("MaSp");

                    b.ToTable("ChiTietPhieuXuat", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.DanhGia", b =>
                {
                    b.Property<int>("MaDanhGia")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaDanhGia"));

                    b.Property<int?>("Diem")
                        .HasColumnType("int");

                    b.Property<string>("HinhAnh")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("MaNguoiDung")
                        .HasColumnType("int");

                    b.Property<int?>("MaSp")
                        .HasColumnType("int")
                        .HasColumnName("MaSP");

                    b.Property<DateTime?>("NgayDanhGia")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("NoiDung")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.HasKey("MaDanhGia");

                    b.HasIndex("MaNguoiDung");

                    b.HasIndex("MaSp");

                    b.ToTable("DanhGia", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.DanhMuc", b =>
                {
                    b.Property<int>("MaDanhMuc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaDanhMuc"));

                    b.Property<string>("MoTa")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("TenDanhMuc")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("MaDanhMuc")
                        .HasName("PK__DanhMuc__B3750887FD221FCC");

                    b.ToTable("DanhMuc", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.DeXuatNhanNuoi", b =>
                {
                    b.Property<int>("MaDeXuat")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaDeXuat"));

                    b.Property<string>("LyDo")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("MaKyGui")
                        .HasColumnType("int");

                    b.Property<int>("MaNhanVienChamSoc")
                        .HasColumnType("int");

                    b.Property<DateOnly?>("NgayDeXuat")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("date")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("TrangThai")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasDefaultValue("Chờ duyệt");

                    b.HasKey("MaDeXuat")
                        .HasName("PK__DeXuatNh__22244765A11BC5AF");

                    b.HasIndex("MaKyGui");

                    b.HasIndex("MaNhanVienChamSoc");

                    b.ToTable("DeXuatNhanNuoi", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.DonHang", b =>
                {
                    b.Property<int>("MaDh")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaDH");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaDh"));

                    b.Property<string>("DiaChiGiao")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("GhiChu")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("HoTenNhan")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int?>("MaHTTT")
                        .HasColumnType("int")
                        .HasColumnName("MaHTTT");

                    b.Property<int?>("MaKm")
                        .HasColumnType("int")
                        .HasColumnName("MaKM");

                    b.Property<int>("MaNguoiDung")
                        .HasColumnType("int");

                    b.Property<int?>("MaPhi")
                        .HasColumnType("int");

                    b.Property<int?>("MaTTTT")
                        .HasColumnType("int")
                        .HasColumnName("MaTTTT");

                    b.Property<string>("MaVanDon")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("NgayDat")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("PhuongThucThanhToan")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("SoDienThoai")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("TenDonViGiao")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal?>("TongTien")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("TrangThai")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasDefaultValue("Chờ duyệt");

                    b.HasKey("MaDh")
                        .HasName("PK__DonHang__27258661E6A07298");

                    b.HasIndex("MaHTTT");

                    b.HasIndex("MaKm");

                    b.HasIndex("MaNguoiDung");

                    b.HasIndex("MaPhi");

                    b.ToTable("DonHang", null, t =>
                        {
                            t.HasTrigger("trg_LuuLichSuTrangThaiDonHang");

                            t.HasTrigger("trg_TaoHoaDonTuDong");
                        });

                    b.HasAnnotation("SqlServer:UseSqlOutputClause", false);
                });

            modelBuilder.Entity("WebPetShop.Models.GiaoHang", b =>
                {
                    b.Property<int>("MaGiaoHang")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaGiaoHang"));

                    b.Property<string>("DonViGiao")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("GhiChu")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("MaDh")
                        .HasColumnType("int")
                        .HasColumnName("MaDH");

                    b.Property<int?>("MaNhanVienGiao")
                        .HasColumnType("int");

                    b.Property<DateTime?>("NgayGiao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("TrangThai")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasDefaultValue("Đang giao");

                    b.HasKey("MaGiaoHang")
                        .HasName("PK__GiaoHang__81CCF4FD3D95E6AC");

                    b.HasIndex("MaDh");

                    b.HasIndex("MaNhanVienGiao");

                    b.ToTable("GiaoHang", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.GioHang", b =>
                {
                    b.Property<int>("MaGh")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaGH");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaGh"));

                    b.Property<int>("MaNguoiDung")
                        .HasColumnType("int");

                    b.Property<DateTime?>("NgayTao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.HasKey("MaGh")
                        .HasName("PK__GioHang__2725AE85CB90290A");

                    b.HasIndex("MaNguoiDung");

                    b.ToTable("GioHang", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.HinhThucThanhToanThucTe", b =>
                {
                    b.Property<int>("MaHTTT")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaHTTT");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaHTTT"));

                    b.Property<string>("MoTa")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("TenHinhThuc")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("MaHTTT")
                        .HasName("PK_HinhThucThanhToanThucTe");

                    b.ToTable("HinhThucThanhToanThucTe", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.HoaDon", b =>
                {
                    b.Property<int>("MaHd")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaHD");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaHd"));

                    b.Property<string>("GhiChu")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("HinhThuc")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int?>("MaDh")
                        .HasColumnType("int")
                        .HasColumnName("MaDH");

                    b.Property<int>("MaKeToan")
                        .HasColumnType("int");

                    b.Property<int?>("MaKyGui")
                        .HasColumnType("int");

                    b.Property<DateTime?>("NgayLap")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<decimal>("SoTien")
                        .HasColumnType("decimal(18, 2)");

                    b.HasKey("MaHd")
                        .HasName("PK__HoaDon__2725A6E05503E113");

                    b.HasIndex("MaDh");

                    b.HasIndex("MaKeToan");

                    b.HasIndex("MaKyGui");

                    b.ToTable("HoaDon", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.KhoHang", b =>
                {
                    b.Property<int>("MaKho")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaKho"));

                    b.Property<string>("GhiChu")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("LoaiGiaoDich")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int?>("MaNcc")
                        .HasColumnType("int")
                        .HasColumnName("MaNCC");

                    b.Property<int>("MaSp")
                        .HasColumnType("int")
                        .HasColumnName("MaSP");

                    b.Property<DateTime?>("NgayThucHien")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<int>("NguoiThucHien")
                        .HasColumnType("int");

                    b.Property<int>("SoLuong")
                        .HasColumnType("int");

                    b.HasKey("MaKho")
                        .HasName("PK__KhoHang__3BDA9350AA34B1D9");

                    b.HasIndex("MaNcc");

                    b.HasIndex("MaSp");

                    b.HasIndex("NguoiThucHien");

                    b.ToTable("KhoHang", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.KhuyenMai", b =>
                {
                    b.Property<int>("MaKm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaKM");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaKm"));

                    b.Property<decimal?>("GiaTriToiDa")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("MaCode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("MoTa")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<DateOnly?>("NgayBatDau")
                        .HasColumnType("date");

                    b.Property<DateOnly?>("NgayKetThuc")
                        .HasColumnType("date");

                    b.Property<int?>("PhanTramGiam")
                        .HasColumnType("int");

                    b.Property<int?>("SoLanSuDungToiDa")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(1);

                    b.Property<bool?>("TrangThai")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.HasKey("MaKm")
                        .HasName("PK__KhuyenMa__2725CF15441787FD");

                    b.HasIndex(new[] { "MaCode" }, "UQ__KhuyenMa__152C7C5C95B7F5B4")
                        .IsUnique();

                    b.ToTable("KhuyenMai", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.KyGuiThuCung", b =>
                {
                    b.Property<int>("MaKyGui")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaKyGui"));

                    b.Property<string>("GhiChu")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("GiongLoai")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("HinhThucThanhToan")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("LoaiThuCung")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int>("MaKh")
                        .HasColumnType("int")
                        .HasColumnName("MaKH");

                    b.Property<DateOnly?>("NgayGui")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("date")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<DateOnly?>("NgayHetHan")
                        .HasColumnType("date");

                    b.Property<DateTime?>("NgayThanhToan")
                        .HasColumnType("datetime2");

                    b.Property<decimal?>("PhiKyGui")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("TenThuCung")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal>("TongTien")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("TrangThai")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasDefaultValue("Đang ký gửi");

                    b.Property<string>("TrangThaiDon")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TrangThaiThanhToan")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("Tuoi")
                        .HasColumnType("int");

                    b.HasKey("MaKyGui")
                        .HasName("PK__KyGuiThu__2B9FA9BE7DC16F18");

                    b.HasIndex("MaKh");

                    b.ToTable("KyGuiThuCung", null, t =>
                        {
                            t.HasTrigger("trg_CapNhatTrangThaiKyGui");
                        });

                    b.HasAnnotation("SqlServer:UseSqlOutputClause", false);
                });

            modelBuilder.Entity("WebPetShop.Models.LichLamViec", b =>
                {
                    b.Property<int>("MaLichLam")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLichLam"));

                    b.Property<string>("CaLam")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("GhiChu")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("MaNhanVien")
                        .HasColumnType("int");

                    b.Property<DateOnly>("NgayLam")
                        .HasColumnType("date");

                    b.Property<string>("TrangThai")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("MaLichLam")
                        .HasName("PK__LichLamV__0A7F064AE10E18A7");

                    b.HasIndex("MaNhanVien");

                    b.ToTable("LichLamViec", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.LichSuHeThong", b =>
                {
                    b.Property<int>("MaLichSu")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLichSu"));

                    b.Property<string>("GhiChu")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("HanhDong")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("MaNguoiDung")
                        .HasColumnType("int");

                    b.Property<DateTime?>("NgayThucHien")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.HasKey("MaLichSu")
                        .HasName("PK__LichSuHe__C443222AB55B2896");

                    b.HasIndex("MaNguoiDung");

                    b.ToTable("LichSuHeThong", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.LichSuTrangThaiDonHang", b =>
                {
                    b.Property<int>("MaLichSuDh")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaLichSuDH");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaLichSuDh"));

                    b.Property<string>("GhiChu")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("MaDh")
                        .HasColumnType("int")
                        .HasColumnName("MaDH");

                    b.Property<DateTime>("NgayCapNhat")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<int?>("NguoiThucHien")
                        .HasColumnType("int");

                    b.Property<string>("TrangThaiCu")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TrangThaiMoi")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("MaLichSuDh")
                        .HasName("PK__LichSuTr__1EBCBDEDFA0A6A31");

                    b.HasIndex("MaDh");

                    b.HasIndex("NguoiThucHien");

                    b.ToTable("LichSuTrangThaiDonHang", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.NguoiDung", b =>
                {
                    b.Property<int>("MaNguoiDung")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaNguoiDung"));

                    b.Property<string>("DiaChi")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("Email")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("GioiTinh")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("HoTen")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("MatKhau")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<DateOnly?>("NgaySinh")
                        .HasColumnType("date");

                    b.Property<string>("SoDienThoai")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("TenDangNhap")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<bool?>("TrangThai")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<string>("VaiTro")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.HasKey("MaNguoiDung")
                        .HasName("PK__NguoiDun__C539D7625505B874");

                    b.HasIndex(new[] { "TenDangNhap" }, "UQ__NguoiDun__55F68FC05F4EA5A2")
                        .IsUnique();

                    b.ToTable("NguoiDung", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.NhaCungCap", b =>
                {
                    b.Property<int>("MaNcc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaNCC");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaNcc"));

                    b.Property<string>("DiaChi")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("Email")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("SoDienThoai")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("TenNcc")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("TenNCC");

                    b.HasKey("MaNcc")
                        .HasName("PK__NhaCungC__3A185DEB011488BD");

                    b.ToTable("NhaCungCap", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.PhiGiaoHang", b =>
                {
                    b.Property<int>("MaPhi")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaPhi"));

                    b.Property<string>("DonViGiao")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("KhuVuc")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal?>("PhiCoDinh")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<decimal?>("PhiTheoKm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m);

                    b.Property<string>("ThoiGianUocTinh")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("MaPhi")
                        .HasName("PK__PhiGiaoH__3AE0AA65C312140C");

                    b.ToTable("PhiGiaoHang", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.PhieuNhap", b =>
                {
                    b.Property<int>("MaPn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaPN");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaPn"));

                    b.Property<int?>("MaNcc")
                        .HasColumnType("int")
                        .HasColumnName("MaNCC");

                    b.Property<int?>("MaNhanVien")
                        .HasColumnType("int");

                    b.Property<DateTime?>("NgayNhap")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<decimal?>("TongTien")
                        .HasColumnType("decimal(18, 2)");

                    b.HasKey("MaPn")
                        .HasName("PK__PhieuNha__2725E7F08E9D30DE");

                    b.HasIndex("MaNcc");

                    b.HasIndex("MaNhanVien");

                    b.ToTable("PhieuNhap", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.PhieuXuat", b =>
                {
                    b.Property<int>("MaPx")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaPX");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaPx"));

                    b.Property<int?>("MaDh")
                        .HasColumnType("int")
                        .HasColumnName("MaDH");

                    b.Property<int?>("MaKhachHang")
                        .HasColumnType("int");

                    b.Property<int?>("MaNhanVien")
                        .HasColumnType("int");

                    b.Property<DateTime?>("NgayXuat")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<decimal?>("TongTien")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m);

                    b.HasKey("MaPx")
                        .HasName("PK__PhieuXuat__2725E7CACE181046");

                    b.HasIndex("MaDh");

                    b.HasIndex("MaKhachHang");

                    b.HasIndex("MaNhanVien");

                    b.ToTable("PhieuXuat", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.SanPham", b =>
                {
                    b.Property<int>("MaSp")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaSP");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaSp"));

                    b.Property<decimal>("Gia")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("HinhAnh")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("MaDanhMuc")
                        .HasColumnType("int");

                    b.Property<string>("MoTa")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("SoLuongTon")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<string>("TenSp")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("TenSP");

                    b.HasKey("MaSp")
                        .HasName("PK__SanPham__2725081C3EEE9CD1");

                    b.HasIndex("MaDanhMuc");

                    b.ToTable("SanPham", null, t =>
                        {
                            t.HasTrigger("trg_LichSuHeThong");
                        });

                    b.HasAnnotation("SqlServer:UseSqlOutputClause", false);
                });

            modelBuilder.Entity("WebPetShop.Models.ThanhToanTrucTuyen", b =>
                {
                    b.Property<int>("MaTttt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaTTTT");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaTttt"));

                    b.Property<int>("MaDh")
                        .HasColumnType("int")
                        .HasColumnName("MaDH");

                    b.Property<string>("MaGiaoDich")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("NgayThanhToan")
                        .HasColumnType("datetime2");

                    b.Property<string>("PhuongThuc")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal?>("SoTien")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("TrangThai")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("MaTttt");

                    b.HasIndex("MaDh")
                        .IsUnique();

                    b.ToTable("ThanhToanTrucTuyen", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.ThuCungNhanNuoi", b =>
                {
                    b.Property<int>("MaNhanNuoi")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaNhanNuoi"));

                    b.Property<int>("MaKyGui")
                        .HasColumnType("int");

                    b.Property<DateOnly?>("NgayChuyen")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("date")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("TrangThai")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasDefaultValue("Chưa có người nhận");

                    b.HasKey("MaNhanNuoi")
                        .HasName("PK__ThuCungN__A03CFC07E64A7684");

                    b.HasIndex("MaKyGui");

                    b.ToTable("ThuCungNhanNuoi", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.VDanhSachThuCungNhanNuoi", b =>
                {
                    b.Property<string>("GiongLoai")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("HinhAnh")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("MaBaiDang")
                        .HasColumnType("int");

                    b.Property<string>("MoTa")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<DateOnly?>("NgayDang")
                        .HasColumnType("date");

                    b.Property<string>("TenThuCung")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("TieuDe")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("TrangThaiBaiDang")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TrangThaiKyGui")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int?>("Tuoi")
                        .HasColumnType("int");

                    b.ToTable((string)null);

                    b.ToView("v_DanhSachThuCungNhanNuoi", (string)null);
                });

            modelBuilder.Entity("WebPetShop.Models.YeuThich", b =>
                {
                    b.Property<int>("MaYt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("MaYT");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaYt"));

                    b.Property<int>("MaNguoiDung")
                        .HasColumnType("int");

                    b.Property<int>("MaSp")
                        .HasColumnType("int")
                        .HasColumnName("MaSP");

                    b.Property<DateTime?>("NgayThem")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.HasKey("MaYt")
                        .HasName("PK__YeuThich__272330D453F09BBA");

                    b.HasIndex("MaNguoiDung");

                    b.HasIndex("MaSp");

                    b.ToTable("YeuThich", (string)null);
                });

            modelBuilder.Entity("YeuCauNhanNuoi", b =>
                {
                    b.Property<int>("MaYeuCau")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MaYeuCau"));

                    b.Property<int>("MaBaiDang")
                        .HasColumnType("int");

                    b.Property<int>("MaBaiDangNavigationMaBaiDang")
                        .HasColumnType("int");

                    b.Property<int>("MaKhachHang")
                        .HasColumnType("int");

                    b.Property<int>("MaKhachHangNavigationMaNguoiDung")
                        .HasColumnType("int");

                    b.Property<DateTime>("NgayYeuCau")
                        .HasColumnType("datetime2");

                    b.Property<string>("TrangThai")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("MaYeuCau");

                    b.HasIndex("MaBaiDangNavigationMaBaiDang");

                    b.HasIndex("MaKhachHangNavigationMaNguoiDung");

                    b.ToTable("YeuCauNhanNuois");
                });

            modelBuilder.Entity("WebPetShop.Models.BaiDangNhanNuoi", b =>
                {
                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNguoiTaoNavigation")
                        .WithMany("BaiDangNhanNuois")
                        .HasForeignKey("MaNguoiTao")
                        .IsRequired()
                        .HasConstraintName("FK__BaiDangNh__MaNgu__1332DBDC");

                    b.HasOne("WebPetShop.Models.ThuCungNhanNuoi", "MaThuCungNhanNuoiNavigation")
                        .WithMany("BaiDangNhanNuois")
                        .HasForeignKey("MaThuCungNhanNuoi")
                        .IsRequired()
                        .HasConstraintName("FK__BaiDangNh__MaThu__123EB7A3");

                    b.Navigation("MaNguoiTaoNavigation");

                    b.Navigation("MaThuCungNhanNuoiNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.BaoCaoDoanhThu", b =>
                {
                    b.HasOne("WebPetShop.Models.NguoiDung", "NguoiLapNavigation")
                        .WithMany("BaoCaoDoanhThus")
                        .HasForeignKey("NguoiLap")
                        .HasConstraintName("FK__BaoCaoDoa__Nguoi__282DF8C2");

                    b.Navigation("NguoiLapNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.ChiTietChamSoc", b =>
                {
                    b.HasOne("WebPetShop.Models.KyGuiThuCung", "MaKyGuiNavigation")
                        .WithMany("ChiTietChamSocs")
                        .HasForeignKey("MaKyGui")
                        .IsRequired()
                        .HasConstraintName("FK__ChiTietCh__MaKyG__08B54D69");

                    b.Navigation("MaKyGuiNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.ChiTietDonHang", b =>
                {
                    b.HasOne("WebPetShop.Models.DonHang", "MaDhNavigation")
                        .WithMany("ChiTietDonHangs")
                        .HasForeignKey("MaDh")
                        .IsRequired()
                        .HasConstraintName("FK__ChiTietDon__MaDH__6B24EA82");

                    b.HasOne("WebPetShop.Models.SanPham", "MaSpNavigation")
                        .WithMany("ChiTietDonHangs")
                        .HasForeignKey("MaSp")
                        .IsRequired()
                        .HasConstraintName("FK__ChiTietDon__MaSP__6C190EBB");

                    b.Navigation("MaDhNavigation");

                    b.Navigation("MaSpNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.ChiTietGioHang", b =>
                {
                    b.HasOne("WebPetShop.Models.GioHang", "MaGhNavigation")
                        .WithMany("ChiTietGioHangs")
                        .HasForeignKey("MaGh")
                        .IsRequired()
                        .HasConstraintName("FK__ChiTietGio__MaGH__60A75C0F");

                    b.HasOne("WebPetShop.Models.SanPham", "MaSpNavigation")
                        .WithMany("ChiTietGioHangs")
                        .HasForeignKey("MaSp")
                        .IsRequired()
                        .HasConstraintName("FK__ChiTietGio__MaSP__619B8048");

                    b.Navigation("MaGhNavigation");

                    b.Navigation("MaSpNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.ChiTietPhieuNhap", b =>
                {
                    b.HasOne("WebPetShop.Models.PhieuNhap", "MaPnNavigation")
                        .WithMany("ChiTietPhieuNhaps")
                        .HasForeignKey("MaPn")
                        .HasConstraintName("FK__ChiTietPhi__MaPN__7F2BE32F");

                    b.HasOne("WebPetShop.Models.SanPham", "MaSpNavigation")
                        .WithMany("ChiTietPhieuNhaps")
                        .HasForeignKey("MaSp")
                        .HasConstraintName("FK__ChiTietPhi__MaSP__00200768");

                    b.Navigation("MaPnNavigation");

                    b.Navigation("MaSpNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.ChiTietPhieuXuat", b =>
                {
                    b.HasOne("WebPetShop.Models.PhieuXuat", "MaPxNavigation")
                        .WithMany("ChiTietPhieuXuats")
                        .HasForeignKey("MaPx")
                        .IsRequired()
                        .HasConstraintName("FK_ChiTietPhieuXuat_PhieuXuat");

                    b.HasOne("WebPetShop.Models.SanPham", "MaSpNavigation")
                        .WithMany("ChiTietPhieuXuats")
                        .HasForeignKey("MaSp")
                        .IsRequired()
                        .HasConstraintName("FK_ChiTietPhieuXuat_SanPham");

                    b.Navigation("MaPxNavigation");

                    b.Navigation("MaSpNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.DanhGia", b =>
                {
                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNguoiDungNavigation")
                        .WithMany("DanhGia")
                        .HasForeignKey("MaNguoiDung")
                        .IsRequired()
                        .HasConstraintName("FK_DanhGia_NguoiDung");

                    b.HasOne("WebPetShop.Models.SanPham", "MaSpNavigation")
                        .WithMany("DanhGia")
                        .HasForeignKey("MaSp")
                        .HasConstraintName("FK_DanhGia_SanPham");

                    b.Navigation("MaNguoiDungNavigation");

                    b.Navigation("MaSpNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.DeXuatNhanNuoi", b =>
                {
                    b.HasOne("WebPetShop.Models.KyGuiThuCung", "MaKyGuiNavigation")
                        .WithMany("DeXuatNhanNuois")
                        .HasForeignKey("MaKyGui")
                        .IsRequired()
                        .HasConstraintName("FK__DeXuatNha__MaKyG__17F790F9");

                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNhanVienChamSocNavigation")
                        .WithMany("DeXuatNhanNuois")
                        .HasForeignKey("MaNhanVienChamSoc")
                        .IsRequired()
                        .HasConstraintName("FK__DeXuatNha__MaNha__18EBB532");

                    b.Navigation("MaKyGuiNavigation");

                    b.Navigation("MaNhanVienChamSocNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.DonHang", b =>
                {
                    b.HasOne("WebPetShop.Models.HinhThucThanhToanThucTe", "HinhThucThanhToanThucTeNavigation")
                        .WithMany("DonHangs")
                        .HasForeignKey("MaHTTT")
                        .HasConstraintName("FK_DonHang_HTThanhToanThucTe");

                    b.HasOne("WebPetShop.Models.KhuyenMai", "MaKmNavigation")
                        .WithMany("DonHangs")
                        .HasForeignKey("MaKm");

                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNguoiDungNavigation")
                        .WithMany("DonHangs")
                        .HasForeignKey("MaNguoiDung")
                        .IsRequired();

                    b.HasOne("WebPetShop.Models.PhiGiaoHang", "MaPhiNavigation")
                        .WithMany("DonHangs")
                        .HasForeignKey("MaPhi");

                    b.Navigation("HinhThucThanhToanThucTeNavigation");

                    b.Navigation("MaKmNavigation");

                    b.Navigation("MaNguoiDungNavigation");

                    b.Navigation("MaPhiNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.GiaoHang", b =>
                {
                    b.HasOne("WebPetShop.Models.DonHang", "MaDhNavigation")
                        .WithMany("GiaoHangs")
                        .HasForeignKey("MaDh")
                        .IsRequired()
                        .HasConstraintName("FK__GiaoHang__MaDH__1DB06A4F");

                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNhanVienGiaoNavigation")
                        .WithMany("GiaoHangs")
                        .HasForeignKey("MaNhanVienGiao")
                        .HasConstraintName("FK__GiaoHang__MaNhan__1EA48E88");

                    b.Navigation("MaDhNavigation");

                    b.Navigation("MaNhanVienGiaoNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.GioHang", b =>
                {
                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNguoiDungNavigation")
                        .WithMany("GioHangs")
                        .HasForeignKey("MaNguoiDung")
                        .IsRequired()
                        .HasConstraintName("FK__GioHang__MaNguoi__5CD6CB2B");

                    b.Navigation("MaNguoiDungNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.HoaDon", b =>
                {
                    b.HasOne("WebPetShop.Models.DonHang", "MaDhNavigation")
                        .WithMany("HoaDons")
                        .HasForeignKey("MaDh")
                        .HasConstraintName("FK__HoaDon__MaDH__236943A5");

                    b.HasOne("WebPetShop.Models.NguoiDung", "MaKeToanNavigation")
                        .WithMany("HoaDons")
                        .HasForeignKey("MaKeToan")
                        .IsRequired()
                        .HasConstraintName("FK__HoaDon__MaKeToan__22751F6C");

                    b.HasOne("WebPetShop.Models.KyGuiThuCung", "MaKyGuiNavigation")
                        .WithMany("HoaDons")
                        .HasForeignKey("MaKyGui")
                        .HasConstraintName("FK__HoaDon__MaKyGui__245D67DE");

                    b.Navigation("MaDhNavigation");

                    b.Navigation("MaKeToanNavigation");

                    b.Navigation("MaKyGuiNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.KhoHang", b =>
                {
                    b.HasOne("WebPetShop.Models.NhaCungCap", "MaNccNavigation")
                        .WithMany("KhoHangs")
                        .HasForeignKey("MaNcc")
                        .HasConstraintName("FK__KhoHang__MaNCC__76969D2E");

                    b.HasOne("WebPetShop.Models.SanPham", "MaSpNavigation")
                        .WithMany("KhoHangs")
                        .HasForeignKey("MaSp")
                        .IsRequired()
                        .HasConstraintName("FK__KhoHang__MaSP__75A278F5");

                    b.HasOne("WebPetShop.Models.NguoiDung", "NguoiThucHienNavigation")
                        .WithMany("KhoHangs")
                        .HasForeignKey("NguoiThucHien")
                        .IsRequired()
                        .HasConstraintName("FK__KhoHang__NguoiTh__778AC167");

                    b.Navigation("MaNccNavigation");

                    b.Navigation("MaSpNavigation");

                    b.Navigation("NguoiThucHienNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.KyGuiThuCung", b =>
                {
                    b.HasOne("WebPetShop.Models.NguoiDung", "MaKhNavigation")
                        .WithMany("KyGuiThuCungs")
                        .HasForeignKey("MaKh")
                        .IsRequired()
                        .HasConstraintName("FK__KyGuiThuCu__MaKH__04E4BC85");

                    b.Navigation("MaKhNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.LichLamViec", b =>
                {
                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNhanVienNavigation")
                        .WithMany("LichLamViecs")
                        .HasForeignKey("MaNhanVien")
                        .IsRequired()
                        .HasConstraintName("FK__LichLamVi__MaNha__4A8310C6");

                    b.Navigation("MaNhanVienNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.LichSuHeThong", b =>
                {
                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNguoiDungNavigation")
                        .WithMany("LichSuHeThongs")
                        .HasForeignKey("MaNguoiDung")
                        .IsRequired()
                        .HasConstraintName("FK__LichSuHeT__MaNgu__367C1819");

                    b.Navigation("MaNguoiDungNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.LichSuTrangThaiDonHang", b =>
                {
                    b.HasOne("WebPetShop.Models.DonHang", "MaDhNavigation")
                        .WithMany("LichSuTrangThaiDonHangs")
                        .HasForeignKey("MaDh")
                        .IsRequired()
                        .HasConstraintName("FK__LichSuTran__MaDH__3A4CA8FD");

                    b.HasOne("WebPetShop.Models.NguoiDung", "NguoiThucHienNavigation")
                        .WithMany("LichSuTrangThaiDonHangs")
                        .HasForeignKey("NguoiThucHien")
                        .HasConstraintName("FK__LichSuTra__Nguoi__3B40CD36");

                    b.Navigation("MaDhNavigation");

                    b.Navigation("NguoiThucHienNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.PhieuNhap", b =>
                {
                    b.HasOne("WebPetShop.Models.NhaCungCap", "MaNccNavigation")
                        .WithMany("PhieuNhaps")
                        .HasForeignKey("MaNcc")
                        .HasConstraintName("FK__PhieuNhap__MaNCC__7B5B524B");

                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNhanVienNavigation")
                        .WithMany("PhieuNhaps")
                        .HasForeignKey("MaNhanVien")
                        .HasConstraintName("FK__PhieuNhap__MaNha__7C4F7684");

                    b.Navigation("MaNccNavigation");

                    b.Navigation("MaNhanVienNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.PhieuXuat", b =>
                {
                    b.HasOne("WebPetShop.Models.DonHang", "MaDhNavigation")
                        .WithMany("PhieuXuats")
                        .HasForeignKey("MaDh")
                        .HasConstraintName("FK_PhieuXuat_DonHang");

                    b.HasOne("WebPetShop.Models.NguoiDung", "MaKhachHangNavigation")
                        .WithMany()
                        .HasForeignKey("MaKhachHang")
                        .HasConstraintName("FK_PhieuXuat_KhachHang");

                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNhanVienNavigation")
                        .WithMany()
                        .HasForeignKey("MaNhanVien")
                        .HasConstraintName("FK_PhieuXuat_NhanVien");

                    b.Navigation("MaDhNavigation");

                    b.Navigation("MaKhachHangNavigation");

                    b.Navigation("MaNhanVienNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.SanPham", b =>
                {
                    b.HasOne("WebPetShop.Models.DanhMuc", "MaDanhMucNavigation")
                        .WithMany("SanPhams")
                        .HasForeignKey("MaDanhMuc")
                        .IsRequired()
                        .HasConstraintName("FK__SanPham__MaDanhM__5070F446");

                    b.Navigation("MaDanhMucNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.ThanhToanTrucTuyen", b =>
                {
                    b.HasOne("WebPetShop.Models.DonHang", "MaDhNavigation")
                        .WithOne("ThanhToanTrucTuyenNavigation")
                        .HasForeignKey("WebPetShop.Models.ThanhToanTrucTuyen", "MaDh")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_DonHang_ThanhToanTrucTuyen");

                    b.Navigation("MaDhNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.ThuCungNhanNuoi", b =>
                {
                    b.HasOne("WebPetShop.Models.KyGuiThuCung", "MaKyGuiNavigation")
                        .WithMany("ThuCungNhanNuois")
                        .HasForeignKey("MaKyGui")
                        .IsRequired()
                        .HasConstraintName("FK__ThuCungNh__MaKyG__0D7A0286");

                    b.Navigation("MaKyGuiNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.YeuThich", b =>
                {
                    b.HasOne("WebPetShop.Models.NguoiDung", "MaNguoiDungNavigation")
                        .WithMany("YeuThiches")
                        .HasForeignKey("MaNguoiDung")
                        .IsRequired()
                        .HasConstraintName("FK__YeuThich__MaNguo__31B762FC");

                    b.HasOne("WebPetShop.Models.SanPham", "MaSpNavigation")
                        .WithMany("YeuThiches")
                        .HasForeignKey("MaSp")
                        .IsRequired()
                        .HasConstraintName("FK__YeuThich__MaSP__32AB8735");

                    b.Navigation("MaNguoiDungNavigation");

                    b.Navigation("MaSpNavigation");
                });

            modelBuilder.Entity("YeuCauNhanNuoi", b =>
                {
                    b.HasOne("WebPetShop.Models.BaiDangNhanNuoi", "MaBaiDangNavigation")
                        .WithMany()
                        .HasForeignKey("MaBaiDangNavigationMaBaiDang")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WebPetShop.Models.NguoiDung", "MaKhachHangNavigation")
                        .WithMany()
                        .HasForeignKey("MaKhachHangNavigationMaNguoiDung")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MaBaiDangNavigation");

                    b.Navigation("MaKhachHangNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.DanhMuc", b =>
                {
                    b.Navigation("SanPhams");
                });

            modelBuilder.Entity("WebPetShop.Models.DonHang", b =>
                {
                    b.Navigation("ChiTietDonHangs");

                    b.Navigation("GiaoHangs");

                    b.Navigation("HoaDons");

                    b.Navigation("LichSuTrangThaiDonHangs");

                    b.Navigation("PhieuXuats");

                    b.Navigation("ThanhToanTrucTuyenNavigation");
                });

            modelBuilder.Entity("WebPetShop.Models.GioHang", b =>
                {
                    b.Navigation("ChiTietGioHangs");
                });

            modelBuilder.Entity("WebPetShop.Models.HinhThucThanhToanThucTe", b =>
                {
                    b.Navigation("DonHangs");
                });

            modelBuilder.Entity("WebPetShop.Models.KhuyenMai", b =>
                {
                    b.Navigation("DonHangs");
                });

            modelBuilder.Entity("WebPetShop.Models.KyGuiThuCung", b =>
                {
                    b.Navigation("ChiTietChamSocs");

                    b.Navigation("DeXuatNhanNuois");

                    b.Navigation("HoaDons");

                    b.Navigation("ThuCungNhanNuois");
                });

            modelBuilder.Entity("WebPetShop.Models.NguoiDung", b =>
                {
                    b.Navigation("BaiDangNhanNuois");

                    b.Navigation("BaoCaoDoanhThus");

                    b.Navigation("DanhGia");

                    b.Navigation("DeXuatNhanNuois");

                    b.Navigation("DonHangs");

                    b.Navigation("GiaoHangs");

                    b.Navigation("GioHangs");

                    b.Navigation("HoaDons");

                    b.Navigation("KhoHangs");

                    b.Navigation("KyGuiThuCungs");

                    b.Navigation("LichLamViecs");

                    b.Navigation("LichSuHeThongs");

                    b.Navigation("LichSuTrangThaiDonHangs");

                    b.Navigation("PhieuNhaps");

                    b.Navigation("YeuThiches");
                });

            modelBuilder.Entity("WebPetShop.Models.NhaCungCap", b =>
                {
                    b.Navigation("KhoHangs");

                    b.Navigation("PhieuNhaps");
                });

            modelBuilder.Entity("WebPetShop.Models.PhiGiaoHang", b =>
                {
                    b.Navigation("DonHangs");
                });

            modelBuilder.Entity("WebPetShop.Models.PhieuNhap", b =>
                {
                    b.Navigation("ChiTietPhieuNhaps");
                });

            modelBuilder.Entity("WebPetShop.Models.PhieuXuat", b =>
                {
                    b.Navigation("ChiTietPhieuXuats");
                });

            modelBuilder.Entity("WebPetShop.Models.SanPham", b =>
                {
                    b.Navigation("ChiTietDonHangs");

                    b.Navigation("ChiTietGioHangs");

                    b.Navigation("ChiTietPhieuNhaps");

                    b.Navigation("ChiTietPhieuXuats");

                    b.Navigation("DanhGia");

                    b.Navigation("KhoHangs");

                    b.Navigation("YeuThiches");
                });

            modelBuilder.Entity("WebPetShop.Models.ThuCungNhanNuoi", b =>
                {
                    b.Navigation("BaiDangNhanNuois");
                });
#pragma warning restore 612, 618
        }
    }
}
